...
the previous revision, [2025-11-25](../2025-11-25/index.md).
...
8. All results now carry a required `resultType` field: `"complete"` for ordinary results and `"input_required"` for [multi round-trip request](basic/patterns/mrtr.md) interim results. Clients **MUST** treat results from earlier-protocol servers that omit the field as `"complete"` ([SEP-2322](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2322)).
...
1. Add `extensions` field to `ClientCapabilities` and `ServerCapabilities` to support optional [extensions](../../extensions/overview.md) beyond the core protocol.
...
11. Remove the `notifications/elicitation/complete` notification and the
    ...
    [Multi Round-Trip Requests](basic/patterns/mrtr.md) pattern, the
...
12. Define an [error code allocation policy](basic.md#error-codes)
...
2. Reclassify the HTTP+SSE transport (deprecated since protocol version
   ...
   Migrate to [Streamable HTTP](basic/transports/streamable-http.md).
...
4. Deprecate the OAuth 2.0 Dynamic Client Registration Protocol
   ...
   [Client ID Metadata Documents](basic/authorization/client-registration.md#client-id-metadata-documents)
...
1. Adopt a specification
   ...
   [registry of deprecated features](deprecated.md)
   ([SEP-2596](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2596)).
