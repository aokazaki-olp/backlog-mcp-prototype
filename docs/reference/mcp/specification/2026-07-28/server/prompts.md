...
[`DiscoverResult`](../schema.md#discoverresult):
...
supports [pagination](utilities/pagination.md) and [caching](utilities/caching.md).
...
auto-completed through [the completion API](utilities/completion.md).
...
Servers **MAY** also respond to `prompts/get` with an [`InputRequiredResult`](../basic/patterns/mrtr.md#inputrequiredresult) to indicate that additional input is needed before the prompt can be resolved. This follows the [multi round-trip requests](../basic/patterns/mrtr.md#multi-round-trip-requests) mechanism. When retrying the request, clients include `inputResponses` and, if provided by the server, `requestState` in the request parameters.
...
<Note>
  ...
  [annotations](resources.md#annotations) for
...
[Resources](resources.md), to provide additional context or
...
Resource links support the same [Resource annotations](resources.md#annotations)
