...
identity. This operation supports [caching](utilities/caching.md).
...
* **stdio backward-compatibility probe.** On stdio, there is no per-request
  ...
  [stdio: Backward Compatibility](../basic/transports/stdio.md#backward-compatibility)
  for the fallback rules.

See [Protocol Version Negotiation](../basic/versioning.md#protocol-version-negotiation)
for the full version-selection flow. For HTTP-specific status codes returned for
unknown methods, see the [Protocol Version Header](../basic/transports/streamable-http.md#protocol-version-header)
