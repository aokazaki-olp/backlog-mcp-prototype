...
[pagination](utilities/pagination.md) and [caching](utilities/caching.md).
...
Servers **MAY** respond to `tools/call` with an [`InputRequiredResult`](../basic/patterns/mrtr.md#inputrequiredresult) to indicate that additional input is needed before the tool call can be completed. This follows the [multi round-trip requests](../basic/patterns/mrtr.md#multi-round-trip-requests) mechanism.
...
[`subscriptions/listen`](../basic/patterns/subscriptions.md) stream with
...
* `inputSchema`: JSON Schema defining expected parameters
  * Follows the [JSON Schema usage guidelines](../basic.md#json-schema-usage)
...
[Streamable HTTP transport](../basic/transports/streamable-http.md#custom-headers-from-tool-parameters).
...
* **MUST** only be applied to properties that are *statically reachable* from the schema
  root, as defined in
  [Custom Headers from Tool Parameters](../basic/transports/streamable-http.md#custom-headers-from-tool-parameters),
...
<Note>
  ...
  support optional
  [annotations](resources.md#annotations) that
...
A tool **MAY** return links to [Resources](resources.md), to provide additional context
...
Resource links support the same [Resource annotations](resources.md#annotations) as regular resources to help clients understand how to use them.
...
[Resources](resources.md) **MAY** be embedded to provide additional context
or data using a suitable [URI scheme](resources.md#common-uri-schemes). Servers that use embedded resources **SHOULD** implement the `resources` capability:
...
Embedded resources support the same [Resource annotations](resources.md#annotations) as regular resources to help clients understand how to use them.
...
1. **Protocol Errors** indicate issues with the request structure itself that models are less likely to be able to fix:
   ...
   * Unknown tool
   * Malformed requests (requests that fail to satisfy [CallToolRequest schema](../schema.md#calltoolrequest))
   * Server errors
...
2. Clients **SHOULD**:
   ...
   * Follow the [`$ref` resolution requirements](../basic.md#ref-resolution)
