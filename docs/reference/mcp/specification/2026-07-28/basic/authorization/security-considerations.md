...
* MCP clients **MUST** include the `resource` parameter in authorization and token requests as specified in the [Resource Parameter Implementation](../authorization.md#resource-parameter-implementation) section
* MCP servers **MUST** validate that tokens presented to them were specifically issued for their use

The [Security Best Practices document](../../../../guides/2026-07-28/tutorials/security/security_best_practices.md#token-passthrough)
...
An attacker that controls one of the authorization servers an MCP client interacts with may attempt to have the client send it an authorization code or token issued by a different, honest authorization server (a mix-up attack, described in [RFC9207 Section 1](https://datatracker.ietf.org/doc/html/rfc9207#section-1)). [Authorization Response Validation](../authorization.md#authorization-response-validation) specifies the required mitigation.
...
When implementing [Client ID Metadata Documents](client-registration.md#client-id-metadata-documents), authorization servers **MUST** consider the security implications
...
Attackers can exploit MCP servers acting as intermediaries to third-party APIs, leading to [confused deputy vulnerabilities](../../../../guides/2026-07-28/tutorials/security/security_best_practices.md#confused-deputy-problem).
By using stolen authorization codes, they can obtain access tokens without user consent.

MCP proxy servers using static client IDs **MUST** obtain user consent for each
[dynamically registered client](client-registration.md#dynamic-client-registration)
...
MCP servers **MUST** only accept tokens specifically intended for themselves and **MUST** reject tokens that do not include them in the audience claim or otherwise verify that they are the intended recipient of the token. See the [Security Best Practices Token Passthrough section](../../../../guides/2026-07-28/tutorials/security/security_best_practices.md#token-passthrough) for details.
