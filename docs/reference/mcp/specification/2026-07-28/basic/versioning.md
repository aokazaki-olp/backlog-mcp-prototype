...
[`_meta`](../basic.md#meta) field. On HTTP, this is
also carried in the
[`MCP-Protocol-Version` header](transports/streamable-http.md#protocol-version-header).
...
[`UnsupportedProtocolVersionError`](../schema.md#unsupportedprotocolversionerror)
...
[`server/discover`](../server/discover.md). Clients
...
[extensions](../../../extensions/overview.md) beyond the core protocol. Extensions
...
**MUST** follow the [`_meta` key naming rules](../basic.md#meta),
with a mandatory prefix.

The following is an example of a client that advertises the
[MCP Apps extension](../../../extensions/apps/overview.md) identified as `io.modelcontextprotocol/ui`:
...
An example of [Tasks extension](../../../extensions/tasks/overview.md) identified as `io.modelcontextprotocol/tasks`:
...
* [stdio](transports/stdio.md#backward-compatibility):
  probe with `server/discover` and fall back on any error that is not a
  recognized modern error.
* [Streamable HTTP](transports/streamable-http.md#backward-compatibility):
