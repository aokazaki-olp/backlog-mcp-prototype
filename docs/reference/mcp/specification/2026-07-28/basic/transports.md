...
[message patterns](patterns.md) are part of the core
protocol and are the same on every binding. The binding pages specify the
standard transports:

1. [stdio](transports/stdio.md): newline-delimited
   messages over the standard streams of a client-launched subprocess.
2. [Streamable HTTP](transports/streamable-http.md):
...
[message patterns](patterns.md), servers do not
...
[`_meta.io.modelcontextprotocol/*`](../basic.md#meta)
fields.
...
[HTTP headers](transports/streamable-http.md#request-metadata)
...
[message patterns](patterns.md), and the per-request
...
[stdio framing](transports/stdio.md) rather than
...
[Versioning: Backward Compatibility](versioning.md#backward-compatibility-with-initialization-based-versions),
