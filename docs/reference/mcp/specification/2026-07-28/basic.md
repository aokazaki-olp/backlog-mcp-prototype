...
version: 2026-07-28
source: "https://modelcontextprotocol.io/specification/2026-07-28/basic"
fetched: 2026-09-06
---
...
[Requests](schema.md#jsonrpcrequest) are sent from the client to the server, to initiate an operation.
...
[Result responses](schema.md#jsonrpcresultresponse) are sent when the operation completes successfully.
...
* A `resultType` of `"input_required"` indicates the request is incomplete and more information is needed to process the request. The result contains an [`InputRequiredResult`](basic/patterns/mrtr.md#inputrequiredresult) object with additional information needed.
...
[Error responses](schema.md#jsonrpcerrorresponse) are sent when the operation fails or encounters an error.
...
* **`-32020` to `-32099` — reserved for the MCP specification.** Error codes
  ...
  recorded in the [schema](schema.md). Implementations
...
| Code     | Name                                                                                                       |
| -------- | ---------------------------------------------------------------------------------------------------------- |
| `-32020` | [`HeaderMismatch`](schema.md#headermismatcherror)                                   |
| `-32021` | [`MissingRequiredClientCapability`](schema.md#missingrequiredclientcapabilityerror) |
| `-32022` | [`UnsupportedProtocolVersion`](schema.md#unsupportedprotocolversionerror)           |
...
* `-32002` — resource not found (2025-11-25 and earlier; replaced by `-32602`).
  ...
  accept `-32002`](server/resources.md#error-handling) from
...
[Notifications](schema.md#jsonrpcnotification) are sent from the client to the server or vice versa, as a one-way message.
...
The Model Context Protocol (MCP) supports several [Message Patterns](basic/patterns.md) that define how clients and servers interact:

1. **[Request and Response](basic/patterns.md#request-and-response)**: A client sends a request to the server, and the server responds with a result or error.
2. **[Multi Round-Trip Requests (MRTR)](basic/patterns.md#multi-round-trip-requests)**: A server requires additional client input (sampling, elicitation, or roots) to complete a request.
3. **[Subscribe and Notify](basic/patterns.md#subscribe-and-notify)**: A client subscribes to a stream of notifications from the server, which are sent as they occur.
...
[`subscriptions/listen`](basic/patterns/subscriptions.md)
...
<Info>
  ...
  [Architecture guide](../../guides/2026-07-28/learn/architecture.md#example).
</Info>

## Auth

MCP provides an [Authorization](basic/authorization.md) framework for use with HTTP.
...
| -------------------------------------------- | ----------------------------------------------------------- | ----------------------------------------------------------------------- |
| `progressToken`                              | Opts the request into progress notifications                | [Progress](basic/patterns/progress.md)           |
...
| `io.modelcontextprotocol/logLevel`           | Minimum log level the server should emit for a request      | [Logging](server/utilities/logging.md)           |
| `io.modelcontextprotocol/subscriptionId`     | Correlates a notification with its originating subscription | [Subscriptions](basic/patterns/subscriptions.md) |
| `traceparent`, `tracestate`, `baggage`       | OpenTelemetry trace context propagation                     | OpenTelemetry trace context (below)                                     |

Official [extensions](basic/versioning.md#extension-negotiation)
...
[`MissingRequiredClientCapabilityError`](schema.md#missingrequiredclientcapabilityerror)
