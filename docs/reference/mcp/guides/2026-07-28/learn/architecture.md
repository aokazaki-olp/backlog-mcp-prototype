...
* [MCP Specification](../../../specification/2026-07-28/index.md): A specification of MCP that outlines the implementation requirements for clients and servers.
* [MCP SDKs](../sdk.md): SDKs for different programming languages that implement MCP.
...
* **Client features**: Enables servers to elicit input from the user. Sampling is [deprecated](../../../specification/2026-07-28/deprecated.md) as of protocol version `2026-07-28`.
...
Elicitation requests are delivered through the [Multi Round-Trip Requests](../../../specification/2026-07-28/basic/patterns/mrtr.md) pattern, explained in the [elicitation overview](client-concepts.md#elicitation).
...
Besides server and client primitives, the protocol supports optional [extensions](../../../extensions/overview.md) that build on the core protocol. For example, the [Tasks extension](../../../extensions/tasks/overview.md) lets servers return a durable handle for long-running requests, so clients can poll for status and retrieve the result later.

#### Notifications

The protocol supports real-time notifications to enable dynamic updates between servers and clients. For example, when a server's available tools change (such as when new functionality becomes available or existing tools are modified), the server can send tool update notifications to inform connected clients about these changes. Notifications are sent as JSON-RPC 2.0 notification messages (without expecting a response). Change notifications are opt-in: the client opens a long-lived [`subscriptions/listen`](../../../specification/2026-07-28/basic/patterns/subscriptions.md) stream naming the notification types it wants to receive, and the server delivers matching notifications on that stream.
...
<Steps>
  <Step title="Discovery">
    ...
    * `"tools": {"listChanged": true}` - The server supports the tools primitive and can honor a `toolsListChanged` filter in [`subscriptions/listen`](../../../specification/2026-07-28/basic/patterns/subscriptions.md). Clients that request this filter receive `notifications/tools/list_changed` when the tool list changes.
  ...
  <Step title="Tool Discovery (Primitives)">
    ...
    The `tools/list` request requires no parameters beyond the standard `_meta` fields that accompany every MCP request. It also accepts an optional `cursor` parameter for [pagination](../../../specification/2026-07-28/server/utilities/pagination.md), which the example above omits.
    ...
    * **`inputSchema`**: A JSON Schema that defines the expected input parameters, enabling type validation and providing clear documentation about required and optional parameters
    ...
    The result is marked `"resultType": "complete"` and carries two caching fields. `ttlMs` is a freshness hint in milliseconds, so this tool list can be cached for five minutes. `cacheScope` indicates who may reuse the response. The specification's [caching utility](../../../specification/2026-07-28/server/utilities/caching.md) defines the full rules.
    ...
    Clients that federate many servers can use [progressive tool discovery](../develop/clients/client-best-practices.md#progressive-tool-discovery) rather than loading every tool upfront.
  </Step>
  ...
  <Step title="Real-time Updates (Notifications)">
    ...
    Change notifications are opt-in. To receive them, the client opens a long-lived notification stream by sending a [`subscriptions/listen`](../../../specification/2026-07-28/basic/patterns/subscriptions.md) request with a `notifications` filter naming the event types it wants. Here the client asks for tool list changes:
