...
The [MCP Inspector](https://github.com/modelcontextprotocol/inspector) is the reference developer tool for testing and debugging [MCP servers](../learn/server-concepts.md). It ships as a single package, `@modelcontextprotocol/inspector`, providing **three clients behind one binary**:
...
All three are built on the same shared core, so a connection behaves identically across them: the same transports, the same configuration files, the same OAuth state on disk, and the same [protocol-era](inspector/protocol-eras.md) negotiation (legacy vs. modern 2026-07-28).
...
<Tabs>
  <Tab title="Web">
    ...
    The command prints a URL containing a one-time session token; open it in your browser. See [Web client](inspector/web.md).
  </Tab>
  ...
  <Tab title="CLI">
    ...
    See [CLI client](inspector/cli.md).
  </Tab>
  ...
  <Tab title="TUI">
    ...
    See [TUI client](inspector/tui.md).
  </Tab>
</Tabs>
...
Everything else (`--catalog`, `--config`, `--server-url`, `--transport`, `--method`, the OAuth flags) is defined by the *client*, not the launcher, and the clients do not all define the same set. The [Configuration and flags](inspector/configuration.md) page is organized that way, by owner.
...
<CardGroup cols={2}>
  <Card title="Web client" icon="browser" href="inspector/web.md">
    A tab-by-tab walkthrough of the graphical inspector.
  </Card>

  <Card title="CLI client" icon="terminal" href="inspector/cli.md">
    Method reference, output formats, exit codes, and CI recipes.
  </Card>

  <Card title="TUI client" icon="table-columns" href="inspector/tui.md">
    Terminal navigation and keyboard reference.
  </Card>

  <Card title="Configuration and flags" icon="sliders" href="inspector/configuration.md">
    Catalog vs. config files, the full per-client flag reference, and
    environment variables.
  </Card>

  <Card title="Authorization" icon="lock" href="inspector/authorization.md">
    The OAuth flow end to end, mid-session re-authorization, and loopback
    callbacks.
  </Card>

  <Card title="Protocol eras" icon="code-branch" href="inspector/protocol-eras.md">
    Legacy vs. modern (2026-07-28) operation, and how every tab changes between
    protocol eras.
  </Card>

  <Card title="Recipes" icon="book" href="inspector/recipes.md">
    Importing client configs, reviewing MCP Apps, Docker, and network hosting.
  </Card>

  <Card title="Debugging guide" icon="bug" href="debugging.md">
