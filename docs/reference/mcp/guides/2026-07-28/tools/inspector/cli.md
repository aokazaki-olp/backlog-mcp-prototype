...
section: guides
version: 2026-07-28
source: "https://modelcontextprotocol.io/docs/2026-07-28/tools/inspector/cli"
fetched: 2026-09-06
---
...
When the server comes from a file, its per-server settings (headers, timeouts, OAuth, [protocol era](protocol-eras.md), and roots) apply to the connection, resolved exactly as the TUI and web client resolve them. A `--header` flag overrides the file's headers for that run while leaving its timeouts and OAuth in place.
...
See [Configuration and flags](configuration.md) for `--catalog` vs. `--config`, the `--` separator, and the shared server-selection flags.
...
`--app-info` reports whether a tool ships an [MCP App](../../../../extensions/apps/overview.md) UI (its `ui://` resource, CSP, and permissions) **without calling the tool**, so a pipeline can decide whether it needs a browser before invoking anything:
...
See [Authorization](authorization.md) for the full flow, the web-to-CLI handoff, and `--print-handoff`.
