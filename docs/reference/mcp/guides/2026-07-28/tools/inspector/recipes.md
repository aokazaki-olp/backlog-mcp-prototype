...
section: guides
version: 2026-07-28
source: "https://modelcontextprotocol.io/docs/2026-07-28/tools/inspector/recipes"
fetched: 2026-09-06
---
...
`--transport` accepts `http` (Streamable HTTP) and `sse`. If the server is protected, see [Authorization](authorization.md): no setup is needed in advance, because when the server answers `401` the Inspector runs the OAuth flow described there and retries the connection.

For an HTTP server, also decide its [protocol era](protocol-eras.md). The default is `legacy`; set `modern` or `auto` in Server Settings (or `protocolEra` in the catalog file) to exercise the 2026-07-28 behavior.
...
Import merges into the active [catalog](configuration.md#choosing-servers)
...
[MCP Apps](../../../../extensions/apps/overview.md) are tools that carry a UI widget. For an automated reviewer (CI or an agent), use the CLI for every check that returns JSON, and open a browser only to inspect the rendered widget.
...
<Steps>
  ...
  <Step title="Navigate one deep link to a rendered widget">
    ...
    `appArgs` is the tool's arguments as base64url-encoded JSON, and every deep-link parameter is described under [Deep links](web.md#deep-links). `autoConnect` and `autoOpen` must both equal the session token, since `autoOpen` fires a tool call straight from the URL and needs the same gate as `autoConnect`.
  </Step>
...
Read the [session token](web.md#the-session-token) from the container logs, or pin it with `-e MCP_INSPECTOR_API_TOKEN=<value>`.
...
Its `HEALTHCHECK` probes the web UI, so add `--no-healthcheck` when running `--cli` or `--tui` (neither has a web server). `<target>` below is an [ad-hoc target](configuration.md#ad-hoc-targets): a positional stdio command, or `--server-url <url> --transport http`.
...
<Steps>
  ...
  <Step title="Lock it in with the CLI">
    ...
    CI](cli.md#verify-a-server-in-ci) for the full
    command.
  </Step>
</Steps>
