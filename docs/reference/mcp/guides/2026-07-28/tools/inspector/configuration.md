...
section: guides
version: 2026-07-28
source: "https://modelcontextprotocol.io/docs/2026-07-28/tools/inspector/configuration"
fetched: 2026-09-06
---
...
The whole scripting surface belongs to the CLI. See [CLI client](cli.md) for usage.
...
| Variable                                  | Effect                                                                                                                         |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `MCP_INSPECTOR_API_TOKEN`                 | Pin the [session token](web.md#the-session-token) instead of generating a random one per launch. |
...
Fields that equal their default are omitted when the Inspector writes the file back, keeping diffs minimal. `protocolEra` (see [Protocol eras](protocol-eras.md)) defaults to `legacy` and `modernLogLevel` to `debug`.

You do not have to hand-write these; the web client can [import an existing client config](recipes.md#importing-an-existing-client-config) from Claude Desktop, Cursor, Cline, or VS Code, or a registry `server.json`.
