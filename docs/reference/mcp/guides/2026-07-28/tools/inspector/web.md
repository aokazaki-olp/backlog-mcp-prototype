...
section: guides
version: 2026-07-28
source: "https://modelcontextprotocol.io/docs/2026-07-28/tools/inspector/web"
fetched: 2026-09-06
---
...
**Network** and **Console** never appear together. Legacy and modern eras are described in [Protocol eras](protocol-eras.md).
...
On a first launch the web client seeds the catalog with two sample servers: a filesystem server scoped to `/tmp` and the canonical "everything" reference server. See [Configuration and flags](configuration.md) for the full rules, including why the CLI and TUI seed an empty catalog instead.

### Server Settings

* **Protocol Era**: `legacy` / `auto` / `modern`. See [Protocol eras](protocol-eras.md).
* **Log level per request**: the level a modern-era connection stamps on each outgoing request by default, or `off` to opt out (see [Logging](protocol-eras.md#logging)).
* **Advertised Extensions**: which extensions the Inspector declares in `capabilities.extensions`. A debugging knob: a server may legitimately change what it registers based on what you advertise. Uncheck the Tasks extension and reconnect against the `test-servers/configs/advertised-extensions-http.json` fixture (setup in [Reproducing each era locally](protocol-eras.md#reproducing-each-era-locally)) to watch a tool disappear.
...
On modern-era servers this screen also shows mirrored `Mcp-Param-*` headers, excluded tools, and distinct `-32602` error panels, all covered in [Protocol eras](protocol-eras.md#tools-mirrored-headers-and-excluded-tools).
...
Lists resources and resource templates with their MIME types and descriptions, reads content on selection, and offers **Subscribe** on servers that support subscriptions. The subscription mechanics differ by era; see [Resource subscriptions](protocol-eras.md#resource-subscriptions).
...
[MCP Apps](../../../../extensions/apps/overview.md) are tools that carry UI. The Apps tab renders one in a sandboxed iframe served from a **separate port**, exercises the `ui/*` bridge, and shows the view's `ui/message` submissions and its `notifications/message` logs in side panels.
...
See [Recipes](recipes.md#reviewing-an-mcp-app) for the CLI-first automated review flow.
...
* **Protocol**: the JSON-RPC transcript. Requests paired with responses, notifications inline, [MRTR](protocol-eras.md#multi-round-tool-results-mrtr) rounds grouped as one conversation, and spec errors rendered by class.
...
A driver (a script, a CI harness, or the CLI's [`--print-handoff`](authorization.md#handing-off-from-the-web-client-to-the-cli)) can reach a *connected* Inspector with a single navigation:
...
See the [Hosting on a network](recipes.md#hosting-on-a-network) recipe for the full matrix, and [Configuration](configuration.md#web-backend-environment-variables) for the variables.
