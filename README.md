...
| `BACKLOG_DOWNLOADS_DIR`    |      | 添付のダウンロードを有効にする。テキストでないものはここへ保存する。ダウンロードフォルダを指すと、チャットへ上げ直しやすい。**未設定ならダウンロードのツールが出ない**（テキストも含めて閉じる。既定は置かない） |
...
- **稼働中に書けなくなったら停止する。** 起動時と同じ理由。ただし黙っては落ちない — 直前の1件には**ツールの結果をそのまま返し**、同じ応答に「監査に記録できないので停止する。その操作は実行された可能性があるので再実行しないこと」を添える。**成功を失敗として返さない** — 再実行されると実行済みの操作が二重に走るため。パスと errno は stderr へ（`起動に失敗しました` ではなく `サーバを停止しました` と出る）
- **ローテーションも自動削除もしない。** 日付でファイルが分かれるだけ。古いログを消すのは運用の判断
- 同期書き込み（プロセスが落ちても直近の行を失わない）。記述子とローテーションは `sonic-boom` に委ねる（pino の書き出し先。レベルの概念を持たない）
...
**未知の名前は起動時マスタに無いので送出する。** 既定に落とさず、選べる名前をメッセージに挙げる。**その名前は第三者が書けるものを含む**ので、囲んでから返す（下の表を見ること）。

**担当者の名前は一意ではない。** ユーザーは `{ id, userId, name }` を持ち、`name` は表示名、`userId` はログイン名。**内部の索引は両方をキーに持つ**ので、衝突は3通りある（表示名どうし／ログイン名と表示名／ログイン名どうし）。**複数の人が名乗る名前は引けなくして、指せる名前を案内する**。黙って先勝ち・後勝ちにすると別人に割り当てることになる。
...
**選べる名前は `list_project_masters` で引ける。** 名前で受ける設計は「何を書けるか分からない」という穴とセットなので、起動時に持っているものをそのまま返すツールを1本置いてある（API へは行かない）。名前だけを返し、数値 ID は返さない。複数の人が名乗っていて指せない名前は、理由を添えて別に返す。
...
**`loginName` は省略されうる。** Backlog には `userId: null` のユーザーが実在する（実データで確認）。**ログイン名が無くても表示名では担当者に指定できる**ので、一覧から落とすと「指定できるのに候補に見えない」状態になる — 数え違えるより重い。`countOf` / `childIssues` と同じ「無いことはキーが無いことで表す」約束に揃えてある。
...
| 中身       | 返すもの                                     | 誰が読むか                                                                                |
| ---------- | -------------------------------------------- | ----------------------------------------------------------------------------------------- |
| テキスト系 | `<untrusted>` で囲んだ本文                   | **LLM。ディスクを触らない**                                                               |
...
**`BACKLOG_DOWNLOADS_DIR` は「保存先」ではなく「ダウンロードの鍵」。** 未設定なら `list_issue_attachments` / `get_issue_attachment` を `tools/list` に出さず、呼んでも拒否する。**テキストだけ使うつもりでも設定は要る** — テキストはディスクを触らないが、それでも「第三者が書いたファイルの中身が、何も設定していない利用者のコンテキストへ入る」ことに変わりはないため。`BACKLOG_ATTACHMENTS_ROOT`（入口の鍵）と対称にしてある。
...
**ポリシーの判定は添付より先に済ませる。** `relatedIssueKey` / `parentIssueKey` の形とスコープは I/O を伴わずに確かめられるので、**上げ切ってから拒否する**順序にしない（上げたファイルは1時間で自動削除されるとはいえ、拒否のためにスペースへ書き込む理由が無い）。

**`BACKLOG_ATTACHMENTS_ROOT` を設定しないと `file` 引数がスキーマに出ない。** 受け付けて実行時に断るのではなく、`tools/list` の段で消す（使えない引数を LLM に見せない）。**ツールごと消すのではなく `file` だけ落とす** — コメント自体は添付が無くても使える。既定を置かないのは、置いた瞬間に「どこが読めるか」が暗黙になるため。MCP 仕様 2026-07-28 で `roots` が非推奨になり（SEP-2577）、クライアントから受け取る道も塞がれているので、設定で明示する以外に安全な決め方が無い。
...
**サーバ自身の設定ファイルは送り出せない。** `BACKLOG_ENV_FILE` / `BACKLOG_ENV_KEYS_FILE` / `BACKLOG_POLICY` が**指しているファイルそのもの**を拒否する。

**主防御は「設定ファイルを添付ルートの外に置くこと」**で、この拒否は2枚目。README の例はポリシーをリポジトリ直下に置くので、作業ディレクトリをルートにすると配下に入りやすい — そのときに効く。

**名前や拡張子では塞がない。** これらのパスは利用者が決めるので、`.env` を `secrets.json` と名付ければ allowlist を通ってしまう（ポリシーの `.json` はそもそも通る）。**照合は2つ。**

| 見るもの                            | 何を捕まえるか                                                             |
| ----------------------------------- | -------------------------------------------------------------------------- |
| **ファイルの識別**（`dev` / `ino`） | 設定ファイルそのもの。symlink や別名で指しても同じ                         |
| **中身**（sha256）                  | **コピーして別名にしたもの**。識別は別物になるので、これが無いと通り抜ける |

識別の照合にパス文字列を使わないのは、Windows がパスの大文字小文字を区別せず、8.3 形式の短い名前でも同じファイルを別の文字列で指せるため。識別が取れない環境（`ino` が `0`）では大文字小文字を無視したパス比較へ落とす。

中身の照合は**サイズが一致したときだけハッシュを取る**ので、ふつうの添付では設定ファイルを読みにすら行かない。**空のファイルは対象外**（中身が無いものは秘密を持たないし、空の添付を拒むのは誤判定にしかならない）。設定ファイルのバイト列は**ハッシュを取る以外の形で持たない**。

**監査ログの出力先も丸ごと拒否する。** 配下に置いてよい添付は無いので、**拡張子で絞らない**（`audit-*.jsonl` を条件にすると、出力の形を変えたときにガードだけが静かに外れる）。判定はルート配下と同じ `path.relative` 方式で、**glob は使わない** — 出力先の名前に `[` が入るだけでパターンが黙ってマッチしなくなるため（`logs[1]` で確認済み）。
...
**扱い方の指示は応答に1回だけ添える。** 囲みが値ごとに運ぶのは境界（`source` と `nonce`）で、同じ応答の中で指示を繰り返しても読み手の行動は変わらない。囲みを含む応答の `content` の末尾に1ブロック足す形にしてあり、ツールの結果（`content[0]`）は JSON のまま残る。**`instructions` には寄せない** — 仕様が `instructions` を「クライアントが使ってもよい」と定めているので、届くかどうかが相手次第になる（監査ログの出口を stderr だけにしないのと同じ理由）。

**囲む対象は「誰が書けるか」で決める。** 本文だけでなく、マスタの**名前**も第三者が書ける（一次情報で確認、2026-09-07）。

| 囲む                                             | 根拠                                                                           |
| ------------------------------------------------ | ------------------------------------------------------------------------------ |
| 課題種別・カテゴリー・マイルストーン・バージョン | 追加も更新も**「すべての権限」**（`add-comment` と同じ）                       |
| Wiki / ドキュメントのタグ                        | ドキュメントタグの追加が**「すべての権限」**                                   |
| リスト型カスタム属性の値                         | **リスト項目の追加**が「すべての権限」（更新・削除だけが管理者）               |
| 担当者・作成者・更新者の表示名                   | **本人が変更できる**（Backlog ヘルプセンター「自分のユーザー情報を変更する」） |

| 囲まない                                                         | 根拠                                                                                                                                                                                                     |
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 状態                                                             | 追加・更新・削除とも**「管理者」**                                                                                                                                                                       |
| 優先度・完了理由                                                 | **書き込む API が無い**（スペース全体で固定）                                                                                                                                                            |
| 添付のファイル名・Wiki のページ名・`list_project_masters` の一覧 | **次の呼び出しに渡す識別子**。囲むと識別子として使えない（2026-09-07 に現状維持と裁定）。**ただしエラー経路の候補列挙は囲む** — そちらは識別子そのものではなく、素のテキストで出ると改行が本物になるため |

囲みの `source`（`backlog:document:設計メモ:title` のような由来）には**第三者が書いた文字列が入る** — Wiki のページ名・Git リポジトリ名・ドキュメントのタイトルはいずれも Backlog の利用者が決める。素通しすると `"` や改行で属性が壊れるので、`wrapUntrusted` の中で許可した文字だけに落としている（**本文の側は落とさない**）。

`source` は組み上がった文字列ではなく `{ subject, name, field }` の3つで受け取る。**第三者が書ける文字列を入れてよいのは `name` だけ**で、長さで削るのもそこだけ（60文字）。項目名（`title` / `content` / `description` …）は削らない — 由来の役目は「どこ由来か示す」ことなので、`backlog:document:…長いタイトル…:t…` のように項目名の側が消えると、同じドキュメントの `title` と `content` を区別できなくなる。
