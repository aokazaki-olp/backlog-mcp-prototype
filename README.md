...
**名前や拡張子では塞がない。** これらのパスは利用者が決めるので、`.env` を `secrets.json` と名付ければ allowlist を通ってしまう（ポリシーの `.json` はそもそも通る）。判定は**開いたハンドルの識別**（`dev` / `ino`）で行い、パス文字列とは比べない — Windows はパスの大文字小文字を区別せず、8.3 形式の短い名前でも同じファイルを別の文字列で指せるため。識別が取れない環境（`ino` が `0`）では大文字小文字を無視したパス比較へ落とす。
