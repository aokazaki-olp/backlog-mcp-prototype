...
const createTransport = (deps) => {
    ...
    return {
        fetch: async (url, options = {}) => {
            ...
            const response = await http(url, fetchOptions);
            ...
            for (let attempt = 0; attempt <= maxRetries; attempt++) {
                try {
                    ...
                    if (attempt === maxRetries) {
                        ...
                        throw new RetryExhaustedError(`リトライ回数上限に達しました${status != null ? ` (HTTP ${status})` : ''}`, { cause: lastError });
                    }
                    const delay = Math.pow(2, attempt) * baseDelayMs;
                    log?.warn(`[HTTP] ⚠ RETRY attempt=${attempt + 1}/${maxRetries}${statusLabel} delay=${delay}ms ${method} ${url}`);
                    await sleep(delay);
                }
            }
