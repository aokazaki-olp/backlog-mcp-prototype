...
export class BacklogApiError extends Error {
    ...
    constructor(message, code, errors, response) {
        super(message);
        this.code = code;
        this.errors = errors;
...
const isBacklogErrorBody = (body) => {
    if (typeof body !== 'object' || body === null) {
        return false;
    }
    // 直前で object かつ非 null を確認済み。errors フィールドを検査するためキャスト
    const errors = body['errors'];
    return Array.isArray(errors) && errors.length > 0 && errors.every((e) => typeof e === 'object' && e !== null
 ...
 */
const withErrorNormalization = (transport) => ({
    fetch: async (url, options) => {
        try {
            ...
            if (e instanceof HttpError && isBacklogErrorBody(e.body)) {
                ...
                throw new BacklogApiError(`Backlog API エラー: ${first.message}`, first.code, e.body.errors, { status: e.status, headers: e.headers, body: e.body, text: e.text });
            }
            throw e;
        }
    },
 ...
 */
const withRetry = (transport, options = {}) => {
 ...
 */
const create = (spaceUrl, auth, options = {}) => {
    ...
    const { maxRetries = DEFAULT_MAX_RETRIES, baseDelayMs = DEFAULT_BASE_DELAY_MS, logger, transport: injectedTransport, } = options;
